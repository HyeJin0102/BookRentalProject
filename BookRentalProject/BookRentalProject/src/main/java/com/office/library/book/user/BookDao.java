...
public class BookDao {
	...
	public int insertTentalBook(int b_no, int u_m_no) {
	...
	public void updateRentalBookAble(int b_no) {
		...
		String sql = "UPDATE tbl_book SET b_rantal_able = 0 WHERE b_no = ?"; //도서번호를 이용해 대출된 책의 b_rantal_able을 0으로 변경
	...
	public List<RentalBookVo> selectRentalBookHistory(int u_m_no) {
		...
		String sql =  "SELECT * FROM tbl_rental_book rb JOIN tbl_book b ON rb.b_no = b.b_no JOIN tbl_user_member um ON rb.u_m_no = um.u_m_no "
					+ "WHERE rb.u_m_no = ? ORDER BY rb.rb_reg_date DESC";
	...
	System.out.println("[BookDao] insertHopeBook()");
		...
		String sql =  "INSERT INTO tbl_hope_book(u_m_no, hb_name, hb_author, hb_publisher, hb_publish_year, hb_reg_date, hb_mod_date, hb_result_last_date) "
					+ "VALUES(?, ?, ?, ?, ?, NOW(), NOW(), NOW())";
		
		int result = -1;
		
		try {
			result = jdbcTemplate.update(sql, hopeBookVo.getU_m_no(), hopeBookVo.getHb_name(), hopeBookVo.getHb_author(), hopeBookVo.getHb_publisher(), 
