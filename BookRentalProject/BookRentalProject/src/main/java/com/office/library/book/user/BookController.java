...
public class BookController {
	...
	public String rentalBookConfirm(@RequestParam("b_no") int b_no, HttpSession session) {
		...
		UserMemberVo loginedUserMemberVo = (UserMemberVo) session.getAttribute("loginedUserMemberVo"); //세션에서 로그인한 사용자의 정보 가져옴
		
//		if(loginedUserMemberVo == null) //사용자가 로그인 상태가 아니면 로그인 화면으로 이동
//			return "redirect:/user/number/loginForm";
		
		int result = bookService.rentalBookConfirm(b_no, loginedUserMemberVo.getU_m_no());
		//도서 대출을 위한 함수 호출. 매개변수로 도서 번호와 사용자 번호 전달함.
