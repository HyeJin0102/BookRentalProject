...
public class BookDAO {
	...
	public int insertBook(BookVo bookVo) {
		...
		String sql = "INSERT INTO tbl_book(b_thumbnail, b_name, b_author, b_publisher, b_publish_year, b_isbn, b_call_number, b_rantal_able, "
				+ "b_reg_date, b_mod_date) VALUES(?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())";
		
		int result = -1;
		
		try {
			result = jdbcTemplate.update(sql, bookVo.getB_thumbnail(), bookVo.getB_name(), bookVo.getB_author(), bookVo.getB_publisher(), 
					bookVo.getB_publish_year(), bookVo.getB_isbn(), bookVo.getB_call_number(), bookVo.getB_rantal_able());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return result;
	...
	public List<RentalBookVo> selectRentalBooks() {
		...
		String sql = "SELECT * FROM tbl_rental_book rb JOIN tbl_book b ON rb.b_no = b.b_no JOIN tbl_user_member um ON rb.u_m_no = um.u_m_no "
				+ "WHERE rb.rb_end_date = '1000-01-01' ORDER BY um.u_m_id ASC, rb.rb_reg_date DESC";
	...
	public List<HopeBookVo> selectHopeBooks() {
		...
		String sql =  "SELECT * FROM tbl_hope_book hb JOIN tbl_user_member um ON hb.u_m_no = um.u_m_no ORDER BY hb.hb_no DESC";
