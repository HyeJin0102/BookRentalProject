...
public class UserMemberDao {
	...
	public int insertUserAccount(UserMemberVo userMemberVo) {
		...
		String sql = "INSERT INTO tbl_user_member(u_m_id, u_m_pw, u_m_name, u_m_gender, u_m_mail, u_m_phone, u_m_reg_date, u_m_mod_date) "
				+ "VALUES(?, ?, ?, ?, ?, ?, NOW(), NOW())";
		
		int result = -1;
		
		try {
			result = jdbcTemplate.update(sql, userMemberVo.getU_m_id(), passwordEncoder.encode(userMemberVo.getU_m_pw()), userMemberVo.getU_m_name(), 
	...
	public int updateUserAccount(UserMemberVo userMemberVo) {
		...
		try {
			result = jdbcTemplate.update(sql, userMemberVo.getU_m_name(), userMemberVo.getU_m_gender(), userMemberVo.getU_m_mail(), 
					userMemberVo.getU_m_phone(), userMemberVo.getU_m_no());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return result;
