...
public class BookService {
	...
	public List<BookVo> searchBookConfirm(BookVo bookVo) {
		System.out.println("[BookServive] searchBookConfirm()");
	...
	public int rentalBookConfirm(int b_no, int rb_no) {
		System.out.println("[BookService] getRentalBooks()");
	...
	public int registerHopeBookConfirm(BookVo bookVo, int hb_no) {
		...
		boolean isISBN = bookDao.isISBN(bookVo.getB_isbn()); //중복 ISBN 조회
		
		if (!isISBN) {
			int result = bookDao.insertBook(bookVo); //데이터베이스에서 신규 도서 등록
			
			if (result > 0) {
				bookDao.updateHopeBookResult(hb_no); //희망 도서 요청 정보를 입고 완료 상태로 업데이트
